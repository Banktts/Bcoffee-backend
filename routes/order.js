...
Router.get("/all/order/:branchId/:date", (req, res) => {
  ...
  if (date == "" && branchId == "all") {
    mysqlConnection.query(
      `select order_id ,customer_id,created_at, updated_at, branch_id,emp_id,totalprice(order_id) as totalpriceb From ordert natural join orderline natural join menu group by order_id`,
      (err, results, fields) => {
        if (!err) {
          res.send(results);
        } else {
    ...
    mysqlConnection.query(
      `select order_id ,customer_id,created_at, updated_at, branch_id,emp_id,totalprice(order_id) as totalpriceb From ordert natural join orderline natural join menu where created_at between "${date} 00:00:00" and "${date} 23:59:59" group by order_id`,
      (err, results, fields) => {
        if (!err) {
          res.send(results);
        } else {
    ...
    mysqlConnection.query(
      `select order_id ,customer_id,created_at, updated_at, branch_id,emp_id,totalprice(order_id) as totalpriceb From ordert natural join orderline natural join menu where branch_id = "${branchId}" and created_at between "${date} 00:00:00" and "${date} 23:59:59" group by order_id`,
      (err, results, fields) => {
        if (!err) {
          res.send(results);
        } else {
